...
buffer);
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        {
            fprintf(stderr, "Error reading range info, no info for ID 
%d\n", i + 1);
            exit(-1);
        }
        mapping_info t_info;
        sscanf(buffer, "%u: %ld %ld", &t_info.id, &t_info.access_low, 
&t_info.access_high);
        t_info.access_high++; // the accessed area is [access_low, 
access_high)
...
mapping_file);
    ...
    while (fgets(buffer, 400, m_file) != NULL)
    {
        ...
        case 'a':
            ...
            break;
        case 'r':
            sscanf(buffer, "r %u %lu %lu %d", &id, &size_1, &size_2, 
&sub_count);
...
void RangeTable::_insertMappings(range_entry &r_info, int p_low, int 
p_high, int a_low, int a_high, int seg_count, bool activate)
{
...
void RangeTable::_readMappings(range_entry &r_info)
{
    ...
    {
        // No insruction in the trace accessed this range, no need to 
record it
    ...
    for (int i = 0; i < m_info.subs.size(); i++)
    {
        ...
        if (i == 0)
            ...
            enable = false;
        _insertMappings(r_info, cur_offset, next_offset, 
m_info.access_low, m_info.access_high, m_info.subs[i].sub_count, enable);
...
void RangeTable::_activateMappings(range_entry &r_info)
{
    ...
    for (int i = 0; i < r_info.subrange.size(); i++)
    {
        uint64_t sub_size = r_info.subrange[i].s_end - 
r_info.subrange[i].s_start;
...
void RangeTable::insertEntry(uint32_t id, uint64_t va_start, uint64_t 
r_size)
{
...
void RangeTable::freeEntry(uint32_t id, uint64_t va_start, uint64_t 
r_size)
{
    int pos = -1;
    for (int i = 0; i < m_entries.size(); i++)
...
void RangeTable::remapEntry(uint32_t id, uint64_t old_va_start, uint64_t 
new_va_start, uint64_t old_r_size, uint64_t new_r_size)
{
    int pos = -1;
    for (int i = 0; i < m_entries.size(); i++)
    {
...
m_entries[pos].subrange[i].s_start, m_entries[pos].subrange[i].s_end);
    ...
    if (sim_cfg.enable_mapping)
    {
        ...
        {
            search_len_bucket = (int*)realloc(search_len_bucket, 
(search_len+1)*sizeof(int));
