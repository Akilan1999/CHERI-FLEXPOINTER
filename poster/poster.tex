...
\begin{columns}
  ...
  \block{Abstract}{
    ...
    % \begin{minipage}[t]{0.6\linewidth}
      ...
      \end{tikzfigure}
      The CHERI hardware architecture is designed to provide security guranetees, This has had performance implications such as more L1 TLB misses on
...
% (This would inturn lookup TLB entries in the range table rather than looking up the L1 TLB entry)
  ...
  \block{Research Questions}{
    ...
    \begin{minipage}[t]{0.8\linewidth}
      ...
      \begin{itemize}
        \item (RQ1) It is possible to use the unused meta-data field in FAT-Pointers to reduce the number of TLB misses to reduce the CPU Stall time ?
  ...
  \block{Expirement Proposal}{
    ...
    % \vspace{0.5cm}
    % \textbf{\textit{RTLB prototype location}}

    % \vspace{0.5cm}
    % \textbf{\textit{Memory object threashold}}
...
\column{0.5}
  ...
  \block{Resources}{
    ...
    \begin{minipage}[t]{0.4\linewidth}
      \vspace{-0.9cm}
      \begin{tikzfigure}{}
        \includegraphics[width=1\textwidth]{Repo-url.png}
      \end{tikzfigure}
    \end{minipage}
